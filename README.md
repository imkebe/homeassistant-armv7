...
Upstream stopped supporting ARMv7 after Home Assistant `2025.11`. Home Assistant `2026.8.2` requires Python `>=3.14.2`, but the official ARMv7 Home Assistant/Python images stop at Python `3.13`.
...
```sh
homeassistant-armv7-lean:2026.8.2
```
...
```sh
homeassistant-armv7-lean:2026.8.2-full
```
...
By default this publishes `ghcr.io/imkebe/homeassistant-armv7:2026.8.2`.
The source image remains `homeassistant-armv7-lean:2026.8.2`; only the public
GHCR name is shortened.

To push the full/debug target:

```sh
IMAGE_TAG=2026.8.2-full ./push.sh
```
...
Or update `docker-compose.yml` to use `homeassistant-armv7-lean:2026.8.2` and run:
...
- Google Calendar requires `pydantic-core` through `ical`; the image builds `pydantic-core==2.46.4` from source and preinstalls `pydantic==2.13.4`, `ical==14.1.0`, `gcal-sync==9.1.0`, and `oauth2client==4.1.3` because no ARMv7 wheel is available.
- Assist requires native voice packages, so the image prebuilds `pymicro-vad==1.0.1`, `pyspeex-noise==1.0.2`, `hassil==3.11.0`, and `home-assistant-intents==2026.7.30`.
- Bluetooth is pinned to the Home Assistant 2026.8.2 requirements: `habluetooth==6.26.5`, `bluetooth-adapters==2.4.0`, `bleak==3.0.2`, and `dbus-fast==5.0.22`.
